...
export function CommunityPulse() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch('/api/community/recent', { cache: 'no-store' });
        if (!res.ok) return;
  ...
  return (
    <div
      aria-live="polite"
      className="pointer-events-none fixed bottom-5 left-5 z-40 max-w-[calc(100vw-2.5rem)] sm:bottom-6 sm:left-6"
    >
      <div
...
function ToastCard({ item, onClose }: { item: Item; onClose: () => void }) {
  const initials = item.first.slice(0, 1).toUpperCase();

  return (
    <div
      className="relative flex items-center gap-3 rounded-[12px] border px-3.5 py-3 pr-9 shadow-[0_10px_32px_-16px_rgba(0,0,0,0.35)]"
      style={{
