...
export default async function FavoritesPage() {
  ...
  const items = await db.postFavorite.findMany({
  ...
  return (
    <div className="space-y-6">
      ...
      {items.length === 0 ? (
        <Empty text="Bir yazının yanındaki yer imi ikonuna basınca burada birikir." />
      ) : (
        <ul className="divide-y" style={{ borderColor: 'var(--border)' }}>
          {items.map((f) => (
            <li key={f.id} className="border-b py-5" style={{ borderColor: 'var(--border)' }}>
