...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await db.newsComment.findUnique({
    where: { id },
    select: { newsItemId: true, status: true },
  });
  ...
  const ops: Prisma.PrismaPromise<unknown>[] = [
    db.newsComment.update({ where: { id }, data: { status: to } }),
  ];
  if (delta !== 0) {
    ops.push(
      db.newsItem.update({
        where: { id: existing.newsItemId },
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const ops: Prisma.PrismaPromise<unknown>[] = [db.newsComment.delete({ where: { id } })];
